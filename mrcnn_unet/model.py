...
#  Unet Graph
############################################################
def u_net_expansive_graph(resnet_feature_maps,numOfClass):
    C1, C2, C3, C4, C5=resnet_feature_maps
    #256,128,64,32,16

    test=KL.UpSampling2D((4,4), interpolation="bilinear")(C1)
    return KL.Conv2D(numOfClass-1, (3, 3), activation="sigmoid",padding="same")(test)
    x = KL.Conv2D(1024, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(C5)      
    x = KL.Conv2D(1024, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)      
    x = KL.UpSampling2D((2,2), interpolation="bilinear")(x)                                 
                                                                        
    x = KL.Concatenate(axis=3,name="unet_concate_C4")([x,C4])                                           
    x = KL.Conv2D(512, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)         
    x = KL.Conv2D(512, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)  
    x = KL.UpSampling2D((2,2), interpolation="bilinear")(x)                                  
                                                                       
    x = KL.Concatenate(axis=3,name="unet_concate_C3")([x,C3])                                            
    x = KL.Conv2D(256, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)         
    x = KL.Conv2D(256, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)    
    x = KL.UpSampling2D((2,2), interpolation="bilinear")(x)                                 
                                                                        
    x = KL.Concatenate(axis=3,name="unet_concate_C2")([x,C2])                                           
    x = KL.Conv2D(128, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)      
    x = KL.Conv2D(128, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)       
    x = KL.UpSampling2D((2,2), interpolation="bilinear")(x) 
                                                                       
                                                                       
    x = KL.Concatenate(axis=3,name="unet_concate_C1")([x,C1])                                           
    x = KL.Conv2D(64, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)        
    x = KL.Conv2D(64, (3,3), padding="same", activation='relu', kernel_initializer = 'he_normal')(x)        

    x = KL.UpSampling2D((4,4), interpolation="bilinear")(x)                             ##這裡要放大四倍

    output = KL.Conv2D(numOfClass-1, (3, 3), activation="sigmoid",padding="same")(x)     #numOfClass 包括背景類別
...
#------------------------------------custom------------------------------------------------
def u_net_mask_loss_graph(input_u_net_mask,u_net_mask):
    loss=K.binary_crossentropy(target=tf.cast(input_u_net_mask,tf.float32), output=tf.cast(u_net_mask,tf.float32))
    loss = K.mean(loss)
    return loss
#------------------------------------------------------------------------------------------
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    #-----------------------------custom------------------------
    u_net_mask = dataset.load_u_net_mask(image_id)
    #-----------------------------------------------------------
    ...
    #-------------------custom----------------------------------------------------------------
    active_class_ids=dataset.active_class_ids
    #-----------------------------------------------------------------------------------------
    ...
    #-----------------------------custom------------------------
    return image, image_meta, class_ids, bbox, mask ,u_net_mask
    #-----------------------------------------------------------
...
class DataGenerator(KU.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        while b < self.batch_size:
            ...
            #---------------------custom--------------------------------------------------------------
            image, image_meta, gt_class_ids, gt_boxes, gt_masks ,gt_u_net_masks= \
                load_image_gt(self.dataset, self.config, image_id,
                              augmentation=self.augmentation)
            #-----------------------------------------------------------------------------------------
            ...
            if b == 0:
                ...
                #---------------------custom--------------------------------------------------------------
                batch_gt_u_net_masks=np.zeros(
                    (self.batch_size,gt_u_net_masks.shape[0],gt_u_net_masks.shape[1],
                    gt_u_net_masks.shape[2]),dtype=gt_u_net_masks.dtype)
                #-----------------------------------------------------------------------------------------
                if self.random_rois:
            ...
            #---------------------custom--------------------------------------------------------------
            batch_gt_u_net_masks[b] = gt_u_net_masks
            #-----------------------------------------------------------------------------------------
            if self.random_rois:
        ...
        #---------------------custom--------------------------------------------------------------
        inputs.append(batch_gt_u_net_masks)
        #-----------------------------------------------------------------------------------------
        outputs = []
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            #-------------------------custom----------------
            input_u_net_mask=KL.Input(shape=[1024,1024,2],name="input_decoder",dtype=tf.int32)  #TODO 這邊shape要改
            #-----------------------------------------------
        ...
        if mode == "training":
            ...
            #---------------------custom--------
            u_net_mask=u_net_expansive_graph([C1,C2,C3,C4,C5],config.NUM_CLASSES)
            #-----------------------------------
            ...
            #----------------------custom-----------
            u_net_mask_loss=KL.Lambda(lambda x:u_net_mask_loss_graph(*x),name="u_net_mask_loss")(
                [input_u_net_mask,u_net_mask])
            #---------------------------------------
            # Model
            inputs = [input_image, input_image_meta,
                      input_rpn_match, input_rpn_bbox, input_gt_class_ids, input_gt_boxes, input_gt_masks]
            #---------------------custom----------
            inputs.append(input_u_net_mask)
            #-------------------------------------
            if not config.USE_RPN_ROIS:
            ...
            #---------------------custom----------
            outputs.append(u_net_mask_loss)
            #-------------------------------------
        ...
        if config.GPU_COUNT > 1:
            from mrcnn_unet.parallel_model import ParallelModel
    ...
    def compile(self, learning_rate, momentum):
        ...
        #-----------------custom-------------------------
        loss_names.append("u_net_mask_loss")
        #------------------------------------------------
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        layer_regex = {
            ...
            #-----------------------custom--------------------
            "all_heads":r"(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)|(u_net\_.*)",
            "u_net":r"(u_net\_.*)",
            #-------------------------------------------------
